...
settings-indicators = Panel badges
settings-indicators-detail = Small marks beside the panel button, for things worth knowing without opening the popup.
settings-indicator-timeout = How long the Wi-Fi badge stays
settings-indicator-side = Which side of the button
indicator-battery = Low battery
indicator-battery-detail = A red battery at { $percent }% or less, unless it is charging.
indicator-wifi = Wi-Fi disconnected
indicator-wifi-detail = A red mark when Wi-Fi drops.
badge-timeout-30s = 30 seconds
badge-timeout-1m = 1 minute
badge-timeout-5m = 5 minutes
